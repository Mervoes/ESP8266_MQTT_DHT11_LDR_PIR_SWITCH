...
#define SCHEDULE_SENSOR_READ 3600 //Segundos
...
void readDhtAndPublish(){
   double humidity = 0;
      double   temperature = 0;
        ...
        Serial.println(temp);
        client.publish("sala/sensor/humidity",hum.c_str(), true);
        client.publish("sala/sensor/temperature",temp.c_str(), true);
}
...
void readLdr(){
 double  lux = 0;
     ...
     lux = light(lux/10);
        ...
        client.publish("salat/sensor/ldr",String(lux).c_str(), true);
}
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    if (checkMqttConnection()){
      ...
      client.loop();
      checkButtonState();
      if (notifTimer.onTimeout(SCHEDULE_SENSOR_READ)){
          readDhtAndPublish();
          readLdr();
      }
      if(OTA){
